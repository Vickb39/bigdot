...
export default function loadDotFile() {
    const commands = []
    for (const obj of data.objects) {
        if (obj._draw_) {
            for (const draw of obj._draw_) {
                const command = getOperation(draw.op as any)
                commands.push(() => command(obj, draw as any))
            }
        }
        if (obj._ldraw_) {
            for (const draw of obj._ldraw_) {
                const command = getOperation(draw.op as any)
                commands.push(() => command(obj, draw as any))
            }
        }
    ...
    for (const edge of data.edges) {
        if (edge._draw_) {
            for (const draw of edge._draw_) {
                const command = getOperation(draw.op as any)
                commands.push(() => command(edge, draw as any))
            }
        }
        if (edge._hdraw_) {
            for (const draw of edge._hdraw_) {
                const command = getOperation(draw.op as any)
                commands.push(() => command(edge, draw as any))
            }
        }
